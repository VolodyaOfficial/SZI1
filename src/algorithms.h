...
public:
    ...
    int manhattan (const vector<int>& board,  int rows);
    int hamming(const std::vector<int>& board, const std::vector<int>& goal);
    ...
    static bool DFSrec(vector<int>& startBoard, int rows, int cols,
                       string& path, int zeroPos, int depth, int maxDepth,
